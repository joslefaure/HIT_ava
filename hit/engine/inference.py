...
def compute_on_dataset_2stage(model, data_loader, device, logger):
    ...
    for i, batch in enumerate(tqdm(data_loader, **extra_args)):
        ...
        for j, (
            ...
            timestamp,
            p_ft,
        ) in enumerate(zip(movie_ids, timestamps, person_feature)):
    ...
    start_time = time.time()
    for movie_ids, timestamps, video_ids, context_feature, det_scores in tqdm(
        batch_info_list, **extra_args
    ):
        current_feat_p = [
            all_feature_pool_p[movie_id, timestamp].to(device)
...
def inference(
    ...
    dataset = data_loader.dataset
    logger.info(
        "Start evaluation on {} dataset({} videos).".format(dataset_name, len(dataset))
    )
    start_time = time.time()
