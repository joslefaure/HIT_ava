...
class MLPFeatureExtractor(nn.Module):
    def __init__(self, config, dim_in):
        ...
        fc1_dim_in = dim_in
        if config.MODEL.HIT_STRUCTURE.ACTIVE and (
            config.MODEL.HIT_STRUCTURE.FUSION == "concat"
        ):
    ...
    def max_pooling_zero_safe(self, x):
        if x.size(0) == 0:
            ...
            x = torch.zeros(
                (0, c, 1, h - res + 1, w - res + 1), device=x.device)
        else:
            x = self.max_pooler(x)
        return x
    ...
    def forward(
        ...
        if part_forward == 1:
            person_pooled = cat([box.get_field("pooled_feature")
                                for box in proposals])
            context_interaction = cat([box.unsqueeze(0) for box in context])
        else:
            # print(slow_features.shape, fast_features.shape)
            context_feats = torch.cat([slow_features.mean(dim=2, keepdim=True), fast_features.mean(dim=2, keepdim=True)], dim=1)
            ...
            context_interaction = torch.rand(
                person_pooled.size(), device="cuda")
        ...
        if ia_active:
            ...
            x_after = self.fusion(
                x_after, ia_feature, self.config.MODEL.HIT_STRUCTURE.FUSION
            )
    ...
    def get_memory_feature(
        ...
        if use_penalty and self.training:
            ...
            cur_loss = 0.0
        current_feat = prepare_pooled_feature(
            current_x, current_box, detach=True)
        for movie_id, timestamp, new_feat in zip(
            extras["movie_ids"], extras["timestamps"], current_feat
        ):
            before_inds = range(timestamp - before *
                                mem_rate, timestamp, mem_rate)
            after_inds = range(
                timestamp + mem_rate, timestamp +
                (after + 1) * mem_rate, mem_rate
            )
            cache_cur_mov = feature_pool[movie_id]
            mem_box_list_before = [
                self.check_fetch_mem_feature(
                    cache_cur_mov, mem_ind, max_boxes, cur_loss, use_penalty
                )
            ...
            mem_feature_list += [
                ...
                if box_list is not None
                    ...
                    0, fixed_dim, 1, 1, 1, dtype=torch.float32, device="cuda"
                )
    ...
    def check_fetch_mem_feature(
        ...
        if use_penalty and self.training:
            ...
            penalty = (
                loss_tag / cur_loss if loss_tag < cur_loss else cur_loss / loss_tag
            )
