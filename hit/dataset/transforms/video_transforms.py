...
class Compose(object):
    ...
    def __call__(self, videos, target):
        ...
        for t in self.transforms:
            videos, target, transform_randoms = t(
                videos, target, transform_randoms)
...
class TemporalCrop(object):
    ...
    def __call__(self, clip, target, transform_randoms):
        ...
        idx = np.arange(frame_start, frame_start +
                        self.frame_num * self.sample_rate, self.sample_rate)
...
class Resize(object):
    ...
    def __call__(self, clip, target, transform_randoms):
        ...
        clip_new = np.zeros(
            (clip.shape[0], size[1], size[0], clip.shape[3]), dtype=np.uint8)
